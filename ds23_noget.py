...
with dataset:
    ...
    # '% de crianças de 11 a 14 anos na escola',
    # '% dos ocupados na indústria de transformação',
    # 'Taxa de frequência ensino médio',
    ...
    # 'Taxa de atividade 15 a 17 anos',
    ...
    for item in lista:
        st.write("- " + item)
    df = get_data("C:/Users/Allan/OneDrive/Área de Trabalho/DS23/data/vendas_predita2.xlsx")#pd.read_excel("C:/Users/Allan/OneDrive/Área de Trabalho/DS23/data/vendas_predita.xlsx")
    df['cod_IBGE'].astype(str)
    # st.write(df.sample(5))
    df_vendas = get_data("C:/Users/Allan/OneDrive/Área de Trabalho/DS23/data/vendas.xlsx")#pd.read_excel("C:/Users/Allan/OneDrive/Área de Trabalho/DS23/data/vendas.xlsx")
    df_vendas = df_vendas[['Cidade','UF','venda_total','periodo']]
    df_vendas['Cidade']=df_vendas['Cidade'].str.upper()
    df_soma = df_vendas.groupby('periodo')['venda_total'].sum()
    # st.write(df_vendas.sample(5))
    st.header('Vendas mensais no período')
...
with model_training:
    ...
    with sel_col:
        ...
        # st.write('Raio selecionado:', km_dist,'km')
    # st.write('Qual o raio em km você deseja obter para compor a região da cidade pesquisada?')
    ...
    with sel_col:
        st.markdown(f"**Venda total  projetada: {total_formatado}**")
    #Criando o objeto do mapa
    mapa = folium.Map(location=[lat_ref, lon_ref], zoom_start=6)
    ...
    #                   tooltip=f'Venda Prevista: R$ {venda_predita:.2f}').add_to(marker_cluster)

    # # Adicionar o objeto MarkerCluster ao mapa
    # marker_cluster.add_to(mapa)

    # # Adicionar um controle de camadas ao mapa
    ...
    #Criando o objeto do mapa
    mapa2 = folium.Map(location=[lat_ref, lon_ref], zoom_start=6)
    data=df[['latitude', 'longitude', 'venda_predita']].copy()
    ipa_loc=(data[(data['latitude']==-19.7992)&(data['longitude']==-41.7164)].index[0])
    data.drop(ipa_loc,inplace=True)
